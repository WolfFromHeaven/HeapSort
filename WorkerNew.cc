...
LineList Worker::FindtopK(LineList array, unsigned int k){
  ...
  LineList charVectorarray[k];
	...
	t1 = clock();
	unsigned int partitionSize = round(array.size()/k);
/*	for(unsigned int i = 0; i < k-1; i++){
		for(unsigned int j = 0; j < partitionSize; j++){
				charVectorarray[i].push_back(array[(i*partitionSize)+j]);
				Id[array[(i*partitionSize)+j]] = i;
			}
	}
	for(unsigned int j = (k-1)*partitionSize; j < array.size(); j++){
			charVectorarray[k-1].push_back(array[j]);
			Id[array[j]] = k-1;
		}*/
...
/*  for(unsigned int i = 0; i < array.size(); i++){
  ...
  for(unsigned int i = 0; i < k-1; i++){
    vector<unsigned char*> temp(next(array.begin(),i*partitionSize), next(array.begin(),(i+1)*partitionSize));
    charVectorarray[i] = temp;
  }
  vector<unsigned char*> temp(next(array.begin(),(k-1)*partitionSize), array.end());
  charVectorarray[k-1] = temp;
  t1 = clock() - t1;
  for(unsigned int i = 0; i < k; i++){
    for (auto it=charVectorarray[i].begin(); it != charVectorarray[i].end(); ++it){
         //cout << ' ' << *it<<" ";
         Id[*it] = i;
         //cout<<Id[*it]<<" ";
    }
